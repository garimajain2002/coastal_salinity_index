# ========================= Test binary EC with Lassp (EC = 2000) ==============================
library(tidyverse)
library(ggplot2)
...
cvmodel <- cv.glmnet(x, y, alpha=1, family='binomial') 
plot(cvmodel)
...
coef(bestlasso)
# selected predictors
# Green, SWIR2, NDWI, NBNIR, NBSWIR2, NRSWIR1, NNIRSWIR1
...
for (i in 1:100) {
  ...
  logistic_model <- tryCatch({
    train(
      EC ~ Red + SWIR1 + SWIR2 + SI4 + VSSI + NBNIR + NBSWIR2, 
  ...
  rf_model <- tryCatch({
    ...
    #   set.seed(123)
    #   print(maxnode)
    #   fit <- train(EC ~ NDWI + Green + SWIR2 + NBR + NBNIR + NBSWIR2 + 
...
write.csv(results_df, "outputs/2000EC/smalldata_modelesults_ECbin_lasso_80_20strat.csv")
...
write.csv(results_df, "outputs/2000EC/smalldata_model_results_ECbin_lasso_80_20random.csv")
