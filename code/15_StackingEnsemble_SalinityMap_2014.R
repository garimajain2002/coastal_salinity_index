...
predicted_EC<- ggplot(predicted_df, aes(x = x, y = y, fill = factor(predicted_EC))) +
  ...
  theme_minimal() +
  labs(title = "Predicted Electrical Conductivity - 2014", 
       fill = "Salinity Class")
...
predicted_Aqua <- ggplot(aqua_df, aes(x = x, y = y, fill = factor(classification))) +
  ...
  theme_minimal() +
  labs(title = "Classified Aquaculture ponds - 2014", 
...
writeRaster(aqua_mask_raster, "outputs/JSP_predicted_Aqua_raster_2014.tif", format = "GTiff", overwrite = TRUE)
...
predicted_ECAqua <- ggplot(final_df, aes(x = x, y = y, fill = factor(predicted_EC))) +
  ...
  theme_minimal() +
  labs(title = "Predicted Electrical Conductivity in Aquaculture context - 2014", 
       fill = "Salinity Class")
...
write.csv(final_df, "outputs/2014_JSP_predicted_ECAqua_df.csv")
