#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
#~~~~~~~~~~~ 2010 Salinity Map using Stacked Ensemble Model ~~~~~~~~~~~~#
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
# 2010 JSP
# landsat_image <- stack(unzip("data/tifs/2010_JSP_Composite_AllBands.zip", "2010_JSP_Composite_AllBands.tif"))
# aqua_image <- stack("data/tifs/2010_JSP_Aquaculture.tif")

# OR

# # 2010 Sampling area
landsat_image <- stack("data/tifs/2010_SA_Composite_AllBands.tif")
aqua_image <- stack("data/tifs/2010_SA_Aquaculture.tif")
...
predicted_EC<- ggplot(predicted_df, aes(x = x, y = y, fill = factor(predicted_EC))) +
  ...
  theme_minimal() +
  labs(title = "Predicted Electrical Conductivity - 2010", 
       fill = "Salinity Class")

plot(predicted_EC)

# Change filename based on original image selection (JSP/SA)
writeRaster(predicted_raster, "outputs/SA_predicted_EC_raster_2010.tif", format = "GTiff", overwrite = TRUE)
ggsave("outputs/SA_predicted_EC_map_2010.png", plot = predicted_EC, width = 8, height = 6, dpi = 300)
...
predicted_Aqua <- ggplot(aqua_df, aes(x = x, y = y, fill = factor(classification))) +
  ...
  theme_minimal() +
  labs(title = "Classified Aquaculture ponds - 2010", 
...
# Change filename based on original image selection (JSP/SA)
ggsave("outputs/SA_predicted_Aqua_map_2010.png", plot = predicted_Aqua, width = 8, height = 6, dpi = 300)
writeRaster(aqua_mask_raster, "outputs/SA_predicted_Aqua_raster_2010.tif", format = "GTiff", overwrite = TRUE)
...
predicted_ECAqua <- ggplot(final_df, aes(x = x, y = y, fill = factor(predicted_EC))) +
  ...
  theme_minimal() +
  labs(title = "Predicted Electrical Conductivity in Aquaculture context - 2010", 
       fill = "Salinity Class")

plot(predicted_ECAqua)

# Change filename based on original image selection (JSP/SA)
ggsave("outputs/SA_predicted_ECAqua_map_2010.png", plot = predicted_ECAqua, width = 8, height = 6, dpi = 300)
writeRaster(predicted_raster, "outputs/SA_predicted_ECAqua_raster_2010.tif", format = "GTiff", overwrite = TRUE)

write.csv(final_df, "outputs/2010_SA_predicted_ECAqua_df.csv")
