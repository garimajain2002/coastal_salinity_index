#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
#~~~~~~~~~~~ 1995 Salinity Map using Stacked Ensemble Model ~~~~~~~~~~~~#
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
# 1995 JSP
# landsat_image <- stack("data/tifs/1995_JSP_Composite_AllBands.tif")
# aqua_image <- stack("data/tifs/1995_JSP_Aquaculture.tif")

# OR 

# # 1995 Sampling area
landsat_image <- stack("data/tifs/1995_SA_Composite_AllBands.tif")
aqua_image <- stack("data/tifs/1995_SA_Aquaculture.tif")
...
predicted_EC<- ggplot(predicted_df, aes(x = x, y = y, fill = factor(predicted_EC))) +
  ...
  theme_minimal() +
  labs(title = "Predicted Electrical Conductivity - 1995", 
       fill = "Salinity Class")

plot(predicted_EC)

# Change filename based on original image selection (JSP/SA)
writeRaster(predicted_raster, "outputs/SA_predicted_EC_raster_1995.tif", format = "GTiff", overwrite = TRUE)
ggsave("outputs/SA_predicted_EC_map_1995.png", plot = predicted_EC, width = 8, height = 6, dpi = 300)
...
# Change filename based on original image selection (JSP/SA)
ggsave("outputs/SA_predicted_Aqua_map_1995.png", plot = predicted_Aqua, width = 8, height = 6, dpi = 300)
writeRaster(aqua_mask_raster, "outputs/SA_predicted_Aqua_raster_1995.tif", format = "GTiff", overwrite = TRUE)
...
predicted_ECAqua <- ggplot(final_df, aes(x = x, y = y, fill = factor(predicted_EC))) +
  ...
  theme_minimal() +
  labs(title = "Predicted Electrical Conductivity in Aquaculture context - 1995", 
       fill = "Salinity Class")

plot(predicted_ECAqua)

# Change filename based on original image selection (JSP/SA)
ggsave("outputs/SA_predicted_ECAqua_map_1995.png", plot = predicted_ECAqua, width = 8, height = 6, dpi = 300)
writeRaster(predicted_raster, "outputs/SA_predicted_ECAqua_raster_1995.tif", format = "GTiff", overwrite = TRUE)

write.csv(final_df, "outputs/1995_SA_predicted_ECAqua_df.csv")
